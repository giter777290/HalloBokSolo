...
export default function Start() {
  ...
  const [danas, setDanas] = useState({ ja: null });
  ...
  useEffect(() => {
    (async () => {
      ...
      const [profili, sviBodovi, sveTeme, sveKarte, napredak, akt, isp] = await Promise.all([
        supabase.from("profile").select("*").eq("user_id", uid),
        supabase.from("punkte").select("*").eq("user_id", uid),
        ...
        supabase.from("aktivitaet").select("*").eq("user_id", uid).gte("datum", prije28),
        supabase.from("pruefungen").select("*").eq("user_id", uid)
      ]);

      const mojProfil = (profili.data || [])[0]
        || { user_id: uid, name: "", akzent: "blau" };
      setJa(mojProfil);
      ...
      /* Tagesbericht */
      setDanas({
        ja: (akt.data || []).find((a) => a.datum === danasnji) || null
      });
  ...
  const Traka = (
    <section className="ploca min-w-0 overflow-hidden p-5">
      <div className="flex items-baseline justify-between">
        <h2 className="text-lg font-semibold tracking-tight">Napredak po nivoima</h2>
            ...
            {n.stanje === "sada" && (
              <>
                ...
                <p className="mt-0.5 text-xs text-tiho">
                  {n.kada ? `još ~${n.kada}` : "vježbajte koji dan za procjenu"}
                </p>
              </>
            )}
            ...
            {otvoren === sek.kljuc && sek.otvorena && (
              <>
                ...
                <p className="mt-1 text-xs text-tiho">
                  ...
                  {sek.popravak.length > 0 &&
                    ` Razina X skuplja ono što je zadavalo muke (${sek.popravak.length}).`}
                </p>
              </>
            )}
          </div>
  ...
  const Radnje = (
    <button onClick={() => router.push("/igre")} className="ploca w-full p-4 text-left">
      ...
      <p className="mt-1 text-xs text-tiho">Brzina i parovi — kratke vježbe</p>
    </button>
  );
